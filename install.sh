...
write_run_makefile() {
  local mk="$1"
  # SC2016: the `$(COMPOSE)` and `$(...)` here are Make variable references that
  # must land in the file LITERALLY — single quotes (no shell expansion) are
  ...
  # shellcheck disable=SC2016
  {
    ...
    printf '.PHONY: docker-pull-oss docker-logs docker-down docker-restart\n\n'
