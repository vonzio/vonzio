 ...
 */
async function generateTitle(prompt: string, response: string, apiKey?: string, log?: { info: Function; error: Function }, provider?: string): Promise<string> {
    ...
    if (sessionId) {
      ...
      (async () => {
        try {
          ...
          const title = await generateTitle(prompt, response, apiKey, wsLog as any, resolved?.resolved_provider);
          if (title && title !== name) {
